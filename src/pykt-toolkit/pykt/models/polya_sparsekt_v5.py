...
# ------------------------------------------------------------------------------
# 3. PolyaSparseKTInteractionEmbedder (New for SparseKT)
# ------------------------------------------------------------------------------
class PolyaSparseKTInteractionEmbedder(nn.Module):
    """
    [Polya for SparseKT]
    Generates:
    ...
    def __init__(
        ...
        debug_print(
            f"PolyaSparseKTInteractionEmbedder (V5 Logic) initialized.",
            fuc_name=model_name,
        )
...
# ------------------------------------------------------------------------------
# 4. PolyaSparseKTNetV5 (Main Network)
# ------------------------------------------------------------------------------
class PolyaSparseKTNetV5(nn.Module):
    def __init__(
        self,
        n_question,
        n_pid,
        ...
        self.model_name = "polya_sparsekt_v5"
        ...
        self.polya_emb = PolyaSparseKTInteractionEmbedder(
            n_question,
            raw_emb_size,
            d_model,
            dropout,
    ...
    def forward(
        ...
        q_fused = self.polya_emb(q_data, target)  # (B, S, D)
        ...
        # 3. Sparse Transformer Processing
...
# ------------------------------------------------------------------------------
# 5. PolyaSparseKTV5 (Wrapper with Aux Loss)
# ------------------------------------------------------------------------------
class PolyaSparseKTV5(QueBaseModel):
    def __init__(
        self,
        n_question,
        n_pid,
        ...
        **kwargs,
        ...
        debug_print(
            f"Initializing PolyaSparseKTV5 (State-Aware MoE + SparseKT + Balancing)...",
            fuc_name=model_name,
        )

        self.model = PolyaSparseKTNetV5(
            n_question=n_question,
            n_pid=n_pid,
            d_model=d_model,
            n_blocks=n_blocks,
...
# ------------------------------------------------------------------------------
# 6. Architecture & Sparse Attention Components (Identical to SparseKTQue)
# ------------------------------------------------------------------------------
class Architecture(nn.Module):
    ...
    def forward(
        ...
        y = qa_embed_data  # Key/Value
        x = q_embed_data  # Query

        # Transformer Blocks with Sparse Attention Args
...
def attention(
    ...
    if emb_type.find("sparseattn") != -1:
        ...
        # Accumulative sparse attention logic
