...
class QueEmb(nn.Module):
    ...
    def forward(self, q, c, r=None):
        ...
        if emb_type == "qaid":
            ...
            que_c_emb = torch.cat([emb_q, emb_c], dim=-1)
            xemb = self.que_c_linear(xemb)
            return xemb, emb_q, emb_c
        elif emb_type == "iekt":
            emb_c = self.get_avg_skill_emb(c)
...
class QueBaseModel(nn.Module):
    ...
    def _save_model(self):
        torch.save(
            ...
            os.path.join(self.save_dir, self.model_name + "_model.ckpt"),
        )

    def load_model(self, save_dir):
        net = torch.load(os.path.join(save_dir, self.model_name + "_model.ckpt"))
    ...
    def _parser_row(self, row, data_config, ob_portions=0.5):
        ...
        start_index, seq_len = self._get_multi_ahead_start_index(
            row["concepts"], ob_portions
        )
    ...
    def _evaluate_multi_ahead_accumulative(
        self, data_config, batch_size=1, ob_portions=0.5, acc_threshold=0.5, max_len=200
    ):
        testf = os.path.join(
            data_config["dpath"], data_config["test_original_file_quelevel"]
        )
        ...
        for i, row in df.iterrows():
            hist_q, hist_c, hist_r, cq_full, cc_full, cr_full, seq_len, start_index = (
                self._parser_row(row, data_config=data_config, ob_portions=ob_portions)
            )
            if i % 100 == 0:
    ...
    def _evaluate_multi_ahead_help(
        self, data_config, batch_size, ob_portions=0.5, acc_threshold=0.5
    ):
        testf = os.path.join(
            data_config["dpath"], data_config["test_original_file_quelevel"]
        )
        ...
        for i, row in df.iterrows():
            ...
            for data in dataloader:
                ...
                y = (
                    self.predict_one_step(data, process=False)[:, -1]
                    .detach()
                    .cpu()
                    .numpy()
                    .flatten()
    ...
    def evaluate_multi_ahead(
        ...
        with torch.no_grad():
            if accumulative:
                ...
                auc, acc = self._evaluate_multi_ahead_accumulative(
                    data_config,
                ...
                auc, acc = self._evaluate_multi_ahead_help(
                    data_config,
                    batch_size=batch_size,
                    ob_portions=ob_portions,
                    acc_threshold=acc_threshold,
