...
def get_class_weights(y, weighted_loss=0):
    if weighted_loss == 0:
        ...
        if not (0 in flat_y_np and 1 in flat_y_np):
            ...
            weight_0, weight_1 = compute_class_weight(
                class_weight="balanced", classes=[0, 1], y=flat_y_np
            )
...
def cal_loss(model, ys, r, rshft, sm, preloss=[], weighted_loss=0):
    ...
    if model_name in ["atdkt", "simplekt", "bakt_time", "sparsekt"]:
        ...
        loss_r = binary_cross_entropy(
            y_curr.float(), r_curr.float()
        )  # if answered wrong for C in t-1, cur answer for C should be wrong too
        ...
        weight_loss = get_class_weights(y, weighted_loss)
        loss = (
            binary_cross_entropy(y.float(), t.float(), weight=weight_loss) + preloss[0]
        )
...
def model_forward(model, data, rel=None, weighted_loss=0):
    ...
    if model_name in ["dkt"]:
        ...
        adv_loss = cal_loss(
            model, [pred_res], r, rshft, sm, weighted_loss=weighted_loss
        )
...
def train_model(
    ...
    for i in range(1, num_epochs + 1):
        ...
        ## YO Edition: Special forward function for CSKT
        if model.model_name == "cskt":
            valid_auc, valid_avg_prc, valid_acc = evaluate_cskt(
                model, valid_loader, model.model_name, rel
            )
            train_auc, train_avg_prc, train_acc = evaluate_cskt(
                model, train_loader, model.model_name, rel
            )
        elif model.model_name == "rkt":
            valid_auc, valid_avg_prc, valid_acc = evaluate(
                model, valid_loader, model.model_name, rel
            )
            train_auc, train_avg_prc, train_acc = evaluate(
                model, train_loader, model.model_name, rel
            )
        else:
            valid_auc, valid_avg_prc, valid_acc = evaluate(
                model, valid_loader, model.model_name
            )
            train_auc, train_avg_prc, train_acc = evaluate(
                model, train_loader, model.model_name
            )
        ### atkt 有diff， 以下代码导致的
        ...
        if valid_auc > max_auc + 2e-4:
            if save_model:
                torch.save(
                    model.state_dict(),
                    os.path.join(ckpt_path, model.model_name + "_model.ckpt"),
                )
            max_auc = valid_auc
            valid_auc_checkpoint, valid_avg_prc_checkpoint, valid_acc_checkpoint = (
                valid_auc,
                valid_avg_prc,
                valid_acc,
            )
            ...
            if not save_model:
                if test_loader != None:
                    ...
                    test_auc, test_avg_prc, test_acc = evaluate(
                ...
                if test_window_loader != None:
                    ...
                    window_test_auc, window_test_avg_prc, window_test_acc = evaluate(
                        model, test_window_loader, model.model_name, save_test_path
                    )

        print(
            f"Epoch: {i}, train_auc: {train_auc:.4}, train_avg_prc: {train_avg_prc:.4}, train_acc: {train_acc:.4}, valid_auc: {valid_auc:.4}, valid_avg_prc: {valid_avg_prc:.4}, valid_acc: {valid_acc:.4}"
        )
        print(
            f"            best epoch: {best_epoch}, best auc: {max_auc:.4}, train loss: {loss_mean}, emb_type: {model.emb_type}, model: {model.model_name}, save_dir: {ckpt_path}"
        )
        print(
            f"            test_auc: {round(test_auc,4)}, test_acc: {round(test_acc,4)}, window_test_auc: {round(window_test_auc,4)}, window_test_acc: {round(window_test_acc,4)}"
        )
