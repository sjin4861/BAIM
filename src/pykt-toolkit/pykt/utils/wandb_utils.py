...
class WandbUtils:
    ...
    def check_sweep_early_stop(
        ...
        if force_check_df:
            df = self.get_df(sweep_id, input_type="sweep_id", only_finish=True)
            report["df"] = df
        ...
        if sweep_status in ["CANCELED", "FINISHED"]:
            ...
            num_run = self.get_sweep_run_num(sweep_id, input_type="sweep_id")
    ...
    def get_sweep_info_by_pattern(
        self,
        sweep_pattern,
        n_jobs=5,
        return_df=False,
        keep_states=["RUNNING", "PENDING", "CANCELED", "FINISHED"],
    ):
        sweep_key_list = []
        for sweep_name in self.sweep_keys:
            if sweep_name.startswith(sweep_pattern) or sweep_pattern == "all":
    ...
    def get_all_fold_name(self, dataset_name, model_name, emb_type="qid"):
        ...
        sweep_key_list = [x for x in sweep_key_list if x in self.sweep_keys]
    ...
    def extract_best_models(
        ...
        ftarget = os.path.join(
            pred_dir,
            "{}_{}_{}_fold_first_predict.yaml".format(
                dataset_name, model_name, emb_type
            ),
        )
        if eval_test:
            self.generate_wandb(
    ...
    def extract_prediction_results(
        ...
        lateall_winauc_mean, lateall_winauc_std = np.mean(late_all_window_aucs), np.std(
            late_all_window_aucs, ddof=0
        )
        lateall_winacc_mean, lateall_winacc_std = np.mean(late_all_window_accs), np.std(
            late_all_window_accs, ddof=0
        )
        key = dataset_name + "_" + model_name
        if print_std:
            print(
