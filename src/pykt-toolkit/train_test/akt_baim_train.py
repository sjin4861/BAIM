...
"""
AKT-BAIM Training Script (State-Aware MoE + Parallel AKT)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="akt_baim")  # BAIM Target
    parser.add_argument("--emb_type", type=str, default="qid")
    parser.add_argument("--save_dir", type=str, default="saved_model_akt_baim")

    # 2. Training Configs
    parser.add_argument(
        "--seed", type=int, default=42
    )  # AKT 원본은 3407이지만, V5 실험 통일성을 위해 42 유지
    ...
    parser.add_argument(
        "--num_attn_heads", type=int, default=8, help="Number of attention heads"
    )
    parser.add_argument(
        "--d_ff",
        type=int,
        default=512,  # [변경] 256 -> 512
        help="Feed-forward dimension",
    )
    parser.add_argument(
        "--final_fc_dim", type=int, default=512, help="Final FC layer dimension"
    )
    parser.add_argument(
        "--kq_same",
        type=int,
    ...
    parser.add_argument(
        "--separate_qa", action="store_true", default=False
    )  # AKTQue에 있는 옵션
