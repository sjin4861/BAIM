...
"""
Polya-QIKT V5 Training Script (State-Aware MoE + QIKT)

Key Features:
1. State-Aware Routing: Parallel GRU tracks student state to guide MoE router.
2. Smart Proxy Content: QIKT's Question Embedding is replaced by Polya-fused embedding.
...
Example Usage:
python polya_qikt_v5_train.py \
  --dataset_name xes3g5m \
  --model_name polya_qikt_v5 \
  --emb_type qid \
  --emb_path /path/to/polya_tensor.pt \
  --flag_load_emb \
  --emb_size 256 \
  --batch_size 128 \
  --use_wandb 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="polya_qikt_v5")
    parser.add_argument("--emb_type", type=str, default="qid")
    parser.add_argument("--save_dir", type=str, default="saved_model_polya_qikt_v5")
