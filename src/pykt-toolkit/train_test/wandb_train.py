...
def main(params):
    ...
    if params["use_wandb"] == 1:
        ...
        model = params.get("model_name", "model")
        dataset = params.get("dataset_name", "dataset")
        ...
        if "wandb_project_name" in params and params["wandb_project_name"] != "":
            wandb.init(project=params["wandb_project_name"], name=run_name)
        else:
    ...
    base_name = f"{dataset_name}_{model_name}_{emb_type}_fold{fold}"
    if params["add_uuid"] == 1 and params["use_wandb"] == 1:
        folder_name = f"{base_name}_{uuid.uuid4().hex[:8]}"  # 짧은 UUID
    else:
    ...
    print(
        "fold\tmodelname\tembtype\ttestauc\ttestavgprc\ttestacc\twindow_testauc\twindow_testavgprc\twindow_testacc\tvalidauc\tvalidavgprc\tvalidacc\tbest_epoch"
    )
    print(
        str(fold)
        + "\t"
        ...
        + emb_type
        + "\t"
        + str(round(dict_res["test_auc"], 4))
        + str(round(dict_res["test_avg_prc"], 4))
        + "\t"
        + str(round(dict_res["test_acc"], 4))
        + "\t"
        + str(round(dict_res["window_test_auc"], 4))
        + str(round(dict_res["window_test_avg_prc"], 4))
        + "\t"
        + str(round(dict_res["window_test_acc"], 4))
        + "\t"
        + str(round(dict_res["valid_auc_checkpoint"], 4))
        + str(round(dict_res["valid_avg_prc_checkpoint"], 4))
        + "\t"
        + str(round(dict_res["valid_acc_checkpoint"], 4))
        + "\t"
